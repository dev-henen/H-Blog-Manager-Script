    ...
    final class Loader {
                ...
                if(count(array_filter($parameters, 'is_array')) > 0) {
                    ...
                    for($i = 0; $i < $length; $i++) {
                        ...
                        foreach($keys as $key) {
                            ...
                            (empty($tmp))? $tmp = @str_replace("<!--{" . $key . "}-->", $value, $get_result) : $tmp = @str_replace("<!--{" . $key . "}-->", $value, $tmp);
                    ...
                    foreach($keys as $key) {
                        ...
                        (empty($tmp))?
                        $tmp = str_replace("<!--{" . $key . "}-->", $value, $get_result)
                        :
        ...
        final public function set(String $parameter_name, String $value) {
            ...
            if(!preg_match("/^[a-zA-Z0-9\.]*$/", $parameter_name)) { 
                ...
                $this->template = str_replace("<!--[" . $parameter_name . "]-->", $value, $this->template);
